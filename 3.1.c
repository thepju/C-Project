...
int main(){
	STD *s1, *s2, *s3, *s4;
	//creatlist
	STDList *plist = (STDList *)malloc(sizeof(STDList));
	plist->count = 0 ;
	plist->First = NULL ;
	plist->Last = NULL ;
	//CreateList1(plist);
	...
	scanf("\n%c",&gender);
	if(dataSet == 0) CreateList0(plist);
	else{
		CreateList1(plist);
		if(dataSet >1 )	CreateListN(plist,dataSet);
	}
	STD *p = plist->First;
	float back=999,breast=999,fly=999,free=999;     // Back Stroke Swimming best time
	float backArr[136],breastArr[136],flyArr[136],freeArr[136];
	int i=0;
