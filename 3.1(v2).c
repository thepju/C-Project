...
void CreateList0(STDList *pList) {

    // reduced of 1
    {STD temp =   {   6110500569,   "Phanupong L.",   'M',   1,   31,   34.42,   36.49,   33.38,   29.26   };    AddList(pList,temp);}
    {STD temp =   {   6110501476,   "Thanaphon Le.",   'M',   1,   25,   26.62,   29.43,   26.04,   23.87   };    AddList(pList,temp);}
    {STD temp =   {   6110506133,   "Isoon A.",   'M',   1,   34,   36.09,   40.49,   35.77,   32.46   };    AddList(pList,temp);}
    {STD temp =   {   6310500023,   "Sirikan Ho.",   'F',   1,   32,   37.8,   39.58,   36.24,   32.75   };    AddList(pList,temp);}
    {STD temp =   {   6310501569,   "Jiratchaya Pu.",   'F',   1,   29,   32.99,   38.64,   30.5,   28.58   };    AddList(pList,temp);}
    {STD temp =   {   6310501666,   "Thanathorn Bu.",   'M',   1,   18,   36.28,   40.2,   31.7,   31.04   };    AddList(pList,temp);}
    {STD temp =   {   6310501721,   "Narawich I.",   'M',   1,   20,   33.88,   37.93,   31.02,   28.3   };    AddList(pList,temp);}
    {STD temp =   {   6310501739,   "Nantawan Jar.",   'F',   1,   35,   42.83,   41.16,   37.06,   36.67   };    AddList(pList,temp);}
    {STD temp =   {   6310501771,   "Pawarut T.",   'M',   1,   20,   35.23,   36.74,   31.71,   28.86   };    AddList(pList,temp);}
    {STD temp =   {   6310501828,   "Patcharamas S.",   'F',   1,   33,   39.82,   41,   36.98,   33.58   };    AddList(pList,temp);}
    {STD temp =   {   6310506498,   "Jirawan May.",   'F',   1,   33,   37.32,   41.57,   35.9,   34.69   };    AddList(pList,temp);}

    {STD temp =   {   6310500589,   "Kanyakorn Pl.",   'F',   2,   28,   31.67,   34.36,   32.25,   27.31   };    AddList(pList,temp);}
    {STD temp =   {   6310500619,   "Thanyawat Cho.",   'M',   2,   26,   28.52,   33.7,   26.14,   24.45   };    AddList(pList,temp);}
    {STD temp =   {   6310500627,   "Porranit K.",   'M',   2,   19,   35.49,   39.45,   32.81,   30.21   };    AddList(pList,temp);}
    {STD temp =   {   6310500635,   "Panuphan S.",   'M',   2,   26,   27.25,   30.45,   27.03,   24.08   };    AddList(pList,temp);}
    {STD temp =   {   6310500651,   "Surisri P.",   'F',   2,   19,   36.73,   39.37,   33.03,   31.71   };    AddList(pList,temp);}
    {STD temp =   {   6310501607,   "Nottakorn S.",   'M',   2,   30,   32.56,   37.2,   31.41,   29.34   };    AddList(pList,temp);}
    {STD temp =   {   6310501763,   "Papatchaya J.",   'F',   2,   25,   28.58,   31.6,   28.6,   24.95   };    AddList(pList,temp);}
    {STD temp =   {   6310502034,   "Atitaya Saej.",   'F',   2,   25,   28.29,   33.88,   27.06,   24.23   };    AddList(pList,temp);}
    {STD temp =   {   6310502051,   "Anut Kl.",   'M',   2,   26,   30.83,   29.59,   27.77,   24.07   };    AddList(pList,temp);}
    {STD temp =   {   6310505386,   "Khanes K.",   'M',   2,   35,   37.45,   39.9,   36.43,   33.17   };    AddList(pList,temp);}
    {STD temp =   {   6310505408,   "Arsee S.",   'F',   2,   28,   33.51,   35.17,   29.31,   29.22   };    AddList(pList,temp);}

    {STD temp =   {   6110551481,   "Nonthawat Ma.",   'M',   250,   24,   28.03,   29.24,   26.66,   23.66   };    AddList(pList,temp);}
    {STD temp =   {   6310550063,   "Chadaporn Ta.",   'F',   250,   26,   29.67,   33.01,   29.68,   26.8   };    AddList(pList,temp);}
    {STD temp =   {   6310550071,   "Chutima Khams.",   'F',   250,   31,   36.26,   41.56,   35.12,   30.69   };    AddList(pList,temp);}
    {STD temp =   {   6310550098,   "Supatchanee C.",   'F',   250,   15,   40.44,   45.39,   39.73,   35.06   };    AddList(pList,temp);}
    {STD temp =   {   6310551400,   "Koravich Ru.",   'M',   250,   25,   26.85,   31.48,   26.62,   22.24   };    AddList(pList,temp);}
    {STD temp =   {   6310551426,   "Kritpeera S.",   'M',   250,   19,   35.77,   34.95,   32.49,   28.85   };    AddList(pList,temp);}
    {STD temp =   {   6310551434,   "Krittiyakorn P.",   'F',   250,   27,   33.98,   33.77,   29.37,   26.5   };    AddList(pList,temp);}
    {STD temp =   {   6310551761,   "Pukjira R.",   'F',   250,   33,   38.44,   41.42,   34.81,   34.11   };    AddList(pList,temp);}
    {STD temp =   {   6310551841,   "Rames M.",   'M',   250,   29,   31.78,   33.35,   28.68,   26.48   };    AddList(pList,temp);}
    {STD temp =   {   6310551949,   "Anon Ji.",   'M',   250,   32,   36.63,   35.87,   31.78,   30.93   };    AddList(pList,temp);}
    {STD temp =   {   6310553941,   "Kom S.",   'M',   250,   23,   28.2,   34.89,   27.96,   24.97   };    AddList(pList,temp);}
    {STD temp =   {   6410551983,   "Nuttanicha Ch.",   'F',   250,   33,   36.8,   39.99,   36.79,   33.35   };    AddList(pList,temp);}

    {STD temp =   {   6010551277,   "Piyawat Pat.",   'M',   251,   34,   38.79,   42.21,   36.57,   32.71   };    AddList(pList,temp);}
    {STD temp =   {   6310550055,   "Chatwimol B.",   'F',   251,   31,   35.17,   40.01,   32.83,   31.01   };    AddList(pList,temp);}
    {STD temp =   {   6310551493,   "Jinjutha Pa.",   'F',   251,   28,   35.17,   37.63,   31.71,   28.2   };    AddList(pList,temp);}
    {STD temp =   {   6310551507,   "Chanathip Suka.",   'M',   251,   25,   29.24,   32.53,   26.52,   22.26   };    AddList(pList,temp);}
    {STD temp =   {   6310551515,   "Chudakarn V.",   'F',   251,   26,   32.99,   35.91,   28.09,   26.41   };    AddList(pList,temp);}
    {STD temp =   {   6310551523,   "Thitiya Srik.",   'F',   251,   32,   35.94,   40.82,   36.34,   33.39   };    AddList(pList,temp);}
    {STD temp =   {   6310551531,   "Napiya S.",   'M',   251,   25,   28.04,   29.26,   24.09,   23.7   };    AddList(pList,temp);}
    {STD temp =   {   6310551558,   "Nicha Run.",   'F',   251,   24,   30.44,   34.19,   29,   26.86   };    AddList(pList,temp);}
    {STD temp =   {   6310551566,   "Nutanyasamorn K.",   'F',   251,   26,   32.33,   35.68,   28.1,   26.13   };    AddList(pList,temp);}
    {STD temp =   {   6310551574,   "Natthanun Thi.",   'M',   251,   17,   38.54,   38.02,   35.21,   31.51   };    AddList(pList,temp);}
    {STD temp =   {   6310551582,   "Nutpakan N.",   'M',   251,   25,   27.35,   31.23,   24.24,   23.96   };    AddList(pList,temp);}
    {STD temp =   {   6310551639,   "Tanakrit Th.",   'M',   251,   24,   27.43,   32.62,   27.92,   23.68   };    AddList(pList,temp);}

    {STD temp =   {   6310001234,   "Michael P.",   'M',   1,   23,   26,   28,   24,   22   };    AddList(pList,temp);}
    {STD temp =   {   6310009999,   "Sarah S.",   'F',   2,   18,   28,   30,   26,   24   };    AddList(pList,temp);}
...
// end of announced codes 1
//============================================================

//SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
// student codes 1 here
...
int main()
{
    ...
    int inputnum, section;
    char gender;
    printf("Enter data set number (0-5): ");
    scanf("%d",&inputnum);
    if (inputnum == 0)
        CreateList0(&CourseList);
    if (inputnum == 1)
        CreateList1(&CourseList);
    if (inputnum >= 2) {
        CreateList1(&CourseList); // !!! som kun mak
        CreateListN(&CourseList,inputnum); // change value
    }

    printf("Enter section (1,2,250,251) or (999 for all sections): ");
    scanf("%d",&section);
...
//    PrintList(&CourseList);
    ...
    /*
        s1 points to STD of the first swimmer (Back50)
        s1 points to STD of the 2nd swimmer (Breastk50)
        s1 points to STD of the 3rd swimmer (Fly50)
        s1 points to STD of the last swimmer (Free50)
    */
    s1 = s2 = s3 = s4 = NULL; // initialize s1 s2 s3 s4

// end of Announced Codes 2
//============================================================

//SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
// student codes 2 here
STD *p = CourseList.First; //first location
 
 int all = CourseList.count;
 float backArr[all],breastArr[all],flyArr[all],freeArr[all];
 int i=0;
 while(p != NULL && section != 999){
  if(p->Gender == gender && p->Sect == section){
   PrintSTD(p);
   backArr[i] = p->Back50;
 ...
 while(q!=NULL && section != 999){
  if(q->Gender == gender && q->Sect == section){
   if(q->Back50==A) s1 = q;
...
// Announced Codes 3
    ...
    if (s1 == NULL || s2 == NULL || s3 == NULL  || s4 == NULL ) {
        printf ("\nNot complete program, please select your swimmers\n");
        exit (0);
    }

    // print out Best Team
    float sum = 0.0;
    printf("\nBest %s Team of section %d\n",(gender=='M')?"Male":"Female",section);
